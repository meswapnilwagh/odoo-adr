...
class pos_config(models.Model):
    ...
    ipf_printer = fields.Boolean("Utiliza impresora fiscal", help="Indica que utiliza impresora fiscal")
...
class pos_order(osv.Model):
    ...
    def _use_ifp_print(self, cr, uid, ids, field, arg, context=None):
        res = {}
        for order in self.browse(cr, uid, ids, context=context):
            res[order.id] = order.session_id.config_id.ipf_printer
    ...
    def create_refund_from_ui(self, cr, uid, tmp_order, context={}):
    ...
    def create_from_ui(self, cr, uid, orders, context={}):
        ...
        for tmp_order in orders_to_update:
            ...
            if payment_session:
                tmp_order["data"]["pos_session_id"] = payment_session
            order_id = self.search(cr, uid, [('pos_reference', '=', tmp_order["id"])])
    ...
    def action_quotation(self, cr, uid, ids, context={}):
        ...
        for order in self.pool.get('pos.order').browse(cr, uid, ids, context=context):
            if order.invoice_id:
                order_ids.append(order.invoice_id.id)
                continue
            ...
            order_dict = {'company_id': order.company_id.id,
                          'fiscal_position': order.partner_id.property_account_position,
                          'incoterm': False,
                          'message_last_post': False,
                          'note': False,
                          'order_policy': u'manual',
                          'origin': order.name,
                          'client_order_ref': order.pos_reference,
                          'partner_id': order.partner_id.id,
                          'partner_invoice_id': order.partner_id.id,
                          'partner_shipping_id': order.partner_id.id,
                          'payment_term': False,
                          'picking_policy': u'direct',
                          'pricelist_id': 1,
                          'project_id': False,
                          'section_id': False,
                          'state': 'draft',
                          'user_id': uid
                          # 'warehouse_id': order.location_id.id
            }

            order_dict.update(sale_ref.onchange_partner_id(cr, uid, [], order.partner_id.id, context=context)['value'])
            ...
            for line in order.lines:
                ...
                order_line.update(sale_line_ref.product_id_change(cr, uid, [], 1, line.product_id.id, qty=line.qty,
                                                                  partner_id=order.partner_id.id)["value"])
                order_line['price_unit'] = line.price_unit
                order_line['discount'] = line.discount
                order_line['name'] = inv_name
                order_line['tax_id'] = [(6, 0, [x.id for x in line.product_id.taxes_id] )]
