function marcos_pos_models(instance, module) {
    ...
    module.PosModel = module.PosModel.extend({
        ...
        validate_user: function (value) {
            ...
            _.each(self.users, function (user) {
                if (user.short_pwd === value) {
                    ...
                    fast_login = true;
                    self.pos_widget.change_user_button.unlock()
                }
            });
        ...
        validate_manager: function() {
            ...
            var current_user = (this.manager >= 1) ? this.manager : this.user.id;
            if (managers) {
        ...
        logout_manager: function () {
            var self = this;
            if (self.manager) {
                _.each(self.users, function (user) {
                    if (user.id === self.user_before_manager) {
                        self.barcode_reader.scan(user.ean13);
                        self.pos_widget.change_user_button.lock();
                        self.manager = false;
                        self.user_before_manager = false;
                        self.manger_validated = false;
                        self.manger_permission = false;
                    }
        ...
        models: [
            {
                ...
                model: 'pos.manager',
                fields: ["max_disc", "change_price", "refund", "cash_refund", "users"],
                domain: null,
